...
export default function ListOfBurners({ btnRef }: { btnRef: any }) {
  ...
  let getTokenMinter = async (startIndex: number, userId?: Principal) => {
    setIsGetting(true);
    let data = await userActor.getListOfBurner(
      userId ?? [],
      startIndex,
    ...
    if (data) {
      ...
      let refinedminters = refinedData(data?.burners);
            ...
            <Row>
              <Col xl='12' lg='12'>
                <div className=''>
                  <Row>
                    <Col xl='8'>
                      <h1>Token Token Burner history </h1>
                    </Col>
                    ...
                    {isGetting || showLoader ? (
                      ...
                      <TableItemOfMinter currentItems={processedList} isBurningList={true}/>
                    ) : (
