...
export default function UserManagment() {
  ...
  const feeSchema = object().shape({
    ...
    amount: number()
      .min(1, "Reward can't less then 1 NFTStudio coin")
      .required('Reward is required')
      .max(1000000000000, 'Reward value cannot be greater than 1000000000000')
  ...
  const sendReward = async (val: any) => {
    ...
    try {
      ...
      let updated = await userActor.give_reward(userid, newReward, true);
      // if (updated)
      ...
      setIsLoading(false);
      childRef?.current?.handleReFetch();
    } catch (error) {
      logger(error);
      setIsLoading(false);
    }
  ...
  return userAuth.userPerms?.adminManagement && !userAuth.isAdminBlocked ? (
    <>
      <main id='main' className='dark'>
            ...
            <Row>
              <Col xl='12' lg='12'>
                <div className=''>
                  <Row className='mb-5'>
                    <Col xl='8' lg='6' md='6'>
                      <h1>Manual Reward Management</h1>
                    </Col>
                  </Row>
                  <div>
                    <Formik
                                ...
                                {({ field, formProps }: any) => (
                                  ...
                                  >
                                        ...
                                        content={
                                          <div>
                                            <p className='mb-0'>
                                              ...
                                              you want to send user and user can
                                              claim these coins.
                                            </p>
                                          </div>
                                        }
                                    ...
                                    <Form.Control
                                      ...
                                      type='number'
                                      name='amount'
                                      placeholder='Enter BlockZa coins amount'
                                    />
                                  </Form.Group>
                                )}
                              </Field>
        ...
        <div>
          <ArtificialAndMenualRewarsList menual={true} btnRef={childRef}/>
        </div>
      </main>
    </>
  ) : (
