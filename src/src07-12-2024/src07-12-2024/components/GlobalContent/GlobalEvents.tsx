...
export default function AllEventsSearch({contentType,setIsData}:{contentType:string,setIsData:any}) {
   ...
   * @return   Events;
   */
  async function getEvents(startIndex:number,str:string) {
    const entryActor = makeEntryActor({
      agentOptions: {
        identity,
      },
    ...
    const resp = await entryActor.get_events(str, startIndex, globalEventsPerPage, [], [], [],tags);
    ...
    if (unEvents.length > 0) {
      ...
      if(refinedEvents && refinedEvents?.lenght==0){
        ...
        setIsData((pre:any)=>({...pre,Events:false}))
      }
  ...
  const handlePageClick = async (event: any) => {
    ...
    const tempEntries = await getEvents(newOffset,searchString ??"");
  ...
  const getNSetEntries = async (str:string) => {
    setIsLoading(true);
    const tempEntries = await getEvents(0,str);
    setIsLoading(false);
  };
